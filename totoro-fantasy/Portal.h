...
private:
    ...
    void drawOval(float centerX, float centerY, float radiusX, float radiusY) {
        ...
        for (int i = 0; i <= 100; i++) {
            ...
            glColor3f(blue, 0.2f * orange, blue + 0.1f * orange);
    ...
    void drawGlow(float centerX, float centerY, float radiusX, float radiusY) {
        glColor4f(0.0f, 0.0f, 1.0f, 0.1f); // Blue glow
    ...
    void drawParticles(float centerX, float centerY, float radiusX, float radiusY) {
        glBegin(GL_POINTS);
...
public:
    Portal()
        : angle(0.0f), glowAngle(0.0f), numParticles(500) { // Fixed number of particles
